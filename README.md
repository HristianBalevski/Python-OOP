...
**Обяснение на правилния пример**
  ...
  - ```Student``` напълно спазва интерфейса на ```Person``` и гарантира, че функционалността на програмата няма да бъде нарушена при замяна на ```Person``` със ```Student```.

**4.Interface Segregation Principle (ISP)**

Класовете, които използват даден интерфейс, не трябва да бъдат принуждавани да имплементират или зависят от методи, които не използват. В Python, въпреки че нямаме официални интерфейси, можем да използваме **миксинообразни класове**, за да разделим различни поведения.
...
## Видове тестване
- **Manual** – извършва се ръчно от човек, без автоматизация.  
- **Automated** – тестове, които се изпълняват автоматично чрез скриптове/инструменти.  
- **Unit** – тестване на най-малката единица код (функция или метод).  
- **Integration** – проверява как отделните модули работят заедно.  
- **E2E (End-to-End)** – тества цялата система от гледна точка на потребителя.  

---

## Какво е Unit тестване?
Unit тестването е процесът на проверка на най-малката независима част от програмата – обикновено **функция** или **метод**.  

---

## Основни термини
- **Test fixture** – средата, в която изпълняваме тестовете.  
- **Test case** – един конкретен тестов сценарий.  
- **Test suite** – колекция от тест кейсове.  
- **Test runner** – инструмент/средство за стартиране на тестовете.  

---

## Triple A Pattern (Arrange – Act – Assert)

Това е най-често използваният подход при писане на unit тестове:  
1. **Arrange** – подготвяме средата, създаваме нужните обекти и стойности.  
2. **Act** – изпълняваме действието, което искаме да тестваме (например извикване на функция).  
3. **Assert** – проверяваме дали резултатът е този, който очакваме.  
...
## 🔹 Какво е мокване (Mocking)?

**Mocking** е техника, при която симулираме дадено поведение или обект, вместо да използваме реалната му имплементация.  

### Използва се, когато:
- Действието е твърде бавно (например заявка към база данни или API).  
- Искаме да избегнем странични ефекти (например изпращане на имейли).  
- Нямаме достъп до реалния ресурс.  

---

## 🔹 Седем принципа на тестването

1. **Изчерпателното тестване е невъзможно** – не можем да покрием всички случаи.  
2. **Ранното тестване е по-добро** – колкото по-рано намерим дефект, толкова по-евтино е да се оправи.  
3. **Defect clustering** – дефектите често са групирани в малък брой модули.  
4. **Pesticide paradox** – ако повтаряме едни и същи тестове, ще спираме да откриваме нови грешки.  
5. **Тестването намалява риска, но не го елиминира** – винаги е възможно да останат бъгове.  
6. **Absence-of-errors fallacy** – липсата на грешки е безполезна, ако софтуерът не отговаря на изискванията на клиента.  
7. **Контекстно зависимо тестване** – различните видове софтуер изискват различни подходи към тестването.  
